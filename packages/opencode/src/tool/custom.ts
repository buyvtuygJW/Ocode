...
// Adding a tool/command changes ONLY this file; the upstream hunks never grow.
// ============================================================================
...
const DESC_DISTILL = "Deterministically distill the CURRENT session into a reusable learned skill using the RSI (Recursive Self-Improvement) heuristic — NO LLM scoring. Reconstructs this session's tool trajectory, scores it 0-100 on Correctness/Efficiency/Coverage/Reproducibility, and only if it scores >= 75 writes a reusable skill to the global opencode skills dir (~/.config/opencode/skills/learned/<name>/SKILL.md), which opencode auto-discovers in future sessions. Run this at the end of a research session. Returns the score breakdown and either the saved skill path or the reason it was skipped.\n"
const DESC_SCHEDULE = "Manage deterministic qm-style background schedules (crons) — plain JSON in the global opencode config dir (schedules.json), NO LLM scoring, NO server. Actions: add (name + every: \"<N>m\"|\"<N>h\"|\"<N>d\"|\"daily@HH:MM\" + prompt [+ agent]) upserts a schedule; list shows all with next-run times; remove deletes by name; due lists the schedules whose time has come (execute each prompt, then call mark); mark records a completed run by name; export prints crontab/schtasks lines that run `opencode run \"<prompt>\"` so the OS scheduler executes them truly unattended. The built-in /schedule command runs everything that is due.\n"
const DESC_COUNCIL = "Convene a multi-model council: fan the same question to several models directly (bypasses the task tool's single-model limit) and combine their answers by strategy. See the `strategy` parameter for the seven modes and their aliases.\n\nMembers come from council.json — a per-project \".opencode/council.json\" overrides the global config dir. Resolution: the inline `members` argument, else the per-project file (a blank template is auto-created; entries with an empty \"model\" are skipped), else the global file.\n\nIf nothing is configured, the tool seeds the global council.json with your current model and prints its path. A council needs at least two models; with one, use the normal task/subagent flow.\n"
const PROMPT_DISTILL = "Call the `distill_session` tool now.\n\nIt deterministically — in code, with NO LLM scoring — reconstructs THIS session's tool\ntrajectory, scores it 0-100 on the RSI rubric (Correctness / Efficiency / Coverage /\nReproducibility), and only if it scores >= 75 saves a reusable learned skill under the global\nopencode skills dir, which opencode auto-discovers in future sessions.\n\nDo NOT score or reconstruct the session yourself. Just invoke `distill_session` and report its\noutput verbatim: the score breakdown, and either the saved skill path or the reason it was not\ndistilled.\n"
const PROMPT_SCHEDULE = "Call the `schedule_task` tool now with action=\"due\".\n\nFor EACH schedule it reports as due, in order:\n1. Execute that schedule's prompt as if the user had just typed it (delegate to its\n   agent via the task tool if one is set).\n2. When it finishes, call `schedule_task` with action=\"mark\" and that schedule's name\n   so the run is recorded.\n\nIf nothing is due, call `schedule_task` with action=\"list\" and report when the next\nschedule will fire.\n\nSchedules are deterministic qm-style crons — plain JSON, no server:\n- add:    schedule_task action=\"add\" name=\"inbox\" every=\"daily@09:00\" prompt=\"triage my inbox\"\n- remove: schedule_task action=\"remove\" name=\"inbox\"\n- export: schedule_task action=\"export\" prints crontab/schtasks lines so the OS runs\n  `opencode run \"<prompt>\"` unattended, without an open session.\n"
const PROMPT_COUNCIL = "Convene the council on the user's request via the `council` tool.\n\n$ARGUMENTS\n\nPass the user's question as `question`. If they named a strategy (council, compare, debate, moa, router, consensus, arena — aliases ok), pass it; else omit. Do not answer yourself first.\n\nRelay the result as-is: the synthesized/resolved answer for council/moa/debate; side-by-side for compare/arena (for arena, ask which to keep); winner + scoreboard for consensus; the chosen model for router.\n\nIf it says not configured, print the file path it returned and tell the user to add two or more \"provider/model\" entries — do not invent models.\n"
...
export const CouncilTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("CouncilTool.execute")(function* (args: Schema.Schema.Type<typeof CouncilParams>, _ctx: Tool.Context) {
      ...
      // validate members against the live provider registry (same lookup ask() uses at runtime).
      // Invalid entries are backfilled with the CURRENT session model — provably working, it is
      // answering right now — never dropped silently. Fewer than 2 distinct survivors -> fail loud
      // with a pick-list of models the user actually has configured.
      const warnings: string[] = []
      {
        ...
        for (const m of members) {
          ...
          const ok = yield* provider.getModel(pm.providerID, pm.modelID).pipe(
            ...
            Effect.catchCause((cause) => {
              ...
              const hint =
                Array.isArray(sq?.suggestions) && sq.suggestions.length
                  ? ` (did you mean: ${sq.suggestions.slice(0, 3).join(", ")}?)`
                  : ""
              bad.push(`  - ${m.name}: ${m.model}${hint}`)
              return Effect.succeed(false)
            }),
          )
          if (ok) checked.push(m)
        }
        if (bad.length) {
          if (cur && !checked.some((c) => c.model === cur)) {
            checked.push({ name: "current", model: cur })
            warnings.push(`${bad.length} invalid member(s) in ${source} — backfilled with the current model (${cur}):\n${bad.join("\n")}`)
          } else {
            warnings.push(
        ...
        members = checked
        if (members.length < (strategy === "router" ? 1 : 2)) {
          const provs = yield* provider.list().pipe(Effect.catchCause(() => Effect.succeed({} as Record<string, { models: Record<string, unknown> }>)))
          const avail: string[] = []
          outer: for (const [pid, info] of Object.entries(provs)) {
            for (const mid of Object.keys(info?.models ?? {})) {
              avail.push(`  - ${pid}/${mid}`)
              if (avail.length >= 12) break outer
            }
          }
          return {
            title: "council: invalid config",
            output: [
              `Too few valid members after validation (${members.length}); a council needs two or more.`,
              bad.length ? `Invalid entries in ${source}:\n${bad.join("\n")}` : "",
              avail.length
                ? `Pick from the models you actually have configured, then edit ${source}:\n${avail.join("\n")}`
                : "No providers are configured — set up API keys first.",
            ]
              .filter(Boolean)
              .join("\n"),
      ...
      if (members.length === 1 && strategy !== "router") {
        return {
          ...
          output: warn + `Only one model is configured (${members[0].model}); a council needs two or more. Edit ${source}, or use the normal task/subagent flow.`,
...
// Loop + object shape live HERE (owned file) so the upstream hunk is a single line
